...
def motorRunWithInputs(angle_inputs):
    ...
    while 1:
        ...
        #Base motor movement looks good
        # ------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        #Simultaneously read position for motors
        sim_move_results, sim_status = SimultaneousReading(1, 2, dxl_goal_inputs)

        #Set the end position values 
        device_index = 1
        while device_index <= 2:
            dxl_end_position[device_index] = sim_move_results[device_index - 1]
            index[device_index] = sim_status[device_index - 1]
            dxl_end_angle[device_index] = _map(dxl_end_position[device_index], 0, 4095, 0, 360)
            device_index += 1
...
def SimultaneousMovement(dxl_goal_inputs):
    ...
    #print("param_goal_bicep_position is ", param_goal_bicep_position)
    #print("param_goal_forearm_position is ", param_goal_forearm_position)
...
def SimultaneousReading(bicep_device_index, forearm_device_index, goal_position):
    def SimuReadMotorData(bicep_index, forearm_index, pos_data_address, len_data_adress):
        ...
        dxl_getdata_result = motor_sync_read.isAvailable(DXL_ID[bicep_index], pos_data_address, len_data_adress)
        if dxl_getdata_result != True:
            print("[ID:%03d] groupSyncRead getdata failed" % DXL_ID[bicep_index])
        # Check if groupsyncread data of Dynamixel Forearm is available
        dxl_getdata_result = motor_sync_read.isAvailable(DXL_ID[forearm_index], pos_data_address, len_data_adress)
        if dxl_getdata_result != True:
            print("[ID:%03d] groupSyncRead getdata failed" % DXL_ID[forearm_index])

        # Get Dynamixel#1 present position value
        dxl1_present_position = motor_sync_read.getData(DXL_ID[bicep_index], pos_data_address, len_data_adress)
        #print("ID:%03d, position = %03d" % (DXL_ID[bicep_index], dxl1_present_position))
        dxl2_present_position = motor_sync_read.getData(DXL_ID[forearm_index], pos_data_address, len_data_adress)
        #print("ID:%03d, position = %03d" % (DXL_ID[forearm_index], dxl2_present_position))

        return dxl1_present_position, dxl2_present_position
    
    print("Goal Position is: " + str(goal_position))
    bicep_repetition_status = 0
    forearm_repetition_status = 0
    bicep_status = 0
    forearm_status = 0
    bicep_present_position, forearm_present_position = SimuReadMotorData(bicep_device_index, forearm_device_index, ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION)
    while 1:
        bicep_new_position, forearm_new_position = SimuReadMotorData(bicep_device_index, forearm_device_index, ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION)
        #print("[ID:%03d] PresPos:%03d  NewPos:%03d" %
              #(DXL_ID[bicep_device_index], bicep_present_position, bicep_new_position))
        #print("[ID:%03d] PresPos:%03d  NewPos:%03d" %
              #(DXL_ID[forearm_device_index], forearm_present_position, forearm_new_position))
    
        if (abs(bicep_new_position - bicep_present_position) < 2) and bicep_status == 0:
            bicep_repetition_status += 1
        else:
            bicep_repetition_status = 0
        
        if (abs(forearm_new_position - forearm_present_position) < 2) and forearm_status == 0:
            forearm_repetition_status += 1
        else:
            forearm_repetition_status = 0

        if (bicep_repetition_status >= 10) or (forearm_repetition_status >= 10):
            break
        
        bicep_present_position = bicep_new_position
        forearm_present_position = forearm_new_position

        if ((abs(goal_position[bicep_device_index] - bicep_present_position) < DXL_MOVING_STATUS_THRESHOLD) and (abs(goal_position[forearm_device_index] - forearm_present_position) < DXL_MOVING_STATUS_THRESHOLD)):
            bicep_status = 1
            forearm_status = 1
            break
        elif abs(goal_position[bicep_device_index] - bicep_present_position) < DXL_MOVING_STATUS_THRESHOLD:
            bicep_status = 1
            forearm_status = 0
        elif abs(goal_position[forearm_device_index] - forearm_present_position) < DXL_MOVING_STATUS_THRESHOLD:
            bicep_status = 0
            forearm_status = 1
        else:
            bicep_status = 0
    ...
    return [bicep_present_position, forearm_present_position], [bicep_status, forearm_status]
...
def motor_check(device_index, goal_position):
    motor_repetition_status = 0
    motor_status = 0
    
    motor_present_position = ReadMotorData(device_index, ADDR_PRESENT_POSITION)
    while 1:
        #Read moving status of motor. If status = 1, motor is still moving. If status = 0, motor stopped moving
        motor_new_position = ReadMotorData(device_index, ADDR_PRESENT_POSITION)
...
"""portInitialization('COM3', 1000000, 1, 1, 3)
...
motorRunWithInputs([200,300,100])
...
motorRunWithInputs([100,200,300])
