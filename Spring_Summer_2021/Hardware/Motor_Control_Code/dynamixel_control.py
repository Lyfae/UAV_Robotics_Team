...
while 1:
    ...
    while not InRange:
        try:
            goal_position_input_degree = int(
                input("Input the goal position in degree (0-360): "))
        except ValueError:
            print("Goal position input needs to be an integer.")
            continue
        else:
            if (goal_position_input_degree < 0) or (goal_position_input_degree > 360):
                print("Goal position is out of range (0-360). Retry.")
                InRange = False
            else:
                goal_position_input = _map(
                    goal_position_input_degree, 0, 360, 0, 4095)
                InRange = True

    # Write goal position
    if (MY_DXL == 'XL320'):  # XL320 uses 2 byte Position Data, Check the size of data in your DYNAMIXEL's control table
        dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
            portHandler, DXL_ID, ADDR_GOAL_POSITION, goal_position_input)
    else:
    ...
    while 1:
        ...
        print("[ID:%03d] GoalPos:%03d  GoalDeg:%03d  PresPos:%03d  PresDeg:%03d" %
              (DXL_ID, goal_position_input, goal_position_input_degree, dxl_present_position, _map(dxl_present_position, 0, 4095, 0, 360)))

        if not abs(goal_position_input - dxl_present_position) > DXL_MOVING_STATUS_THRESHOLD:
            break
